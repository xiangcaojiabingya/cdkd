...
export function isLogin() {
	if (getApp().globalData.iCIFID || uni.getStorageSync("iCIFID")) {
		return true
	} else {
		// try {
...
//银行账号掩码
export function bankAccountMask(bankNo) {
	var output = bankNo;
	if (bankNo == null || bankNo == "")
		return "";
	output = bankNo.trim(bankNo);
	if (bankNo.length > 10) {
		output = bankNo.substr(0, 6) + "****" + bankNo.substr(bankNo.length - 4);
	} else if (bankNo.length > 4) {
		output = bankNo.substr(0, 4) + "****";
...
const gfdGlobalData = {
	getData: function(item) {
		if(typeof(getApp().globalData[item]) === 'boolean'){
			return getApp().globalData[item]
		} else {
			return getApp().globalData[item] || uni.getStorageSync(item)
		}
	},
	setData: function(item, data) {
		getApp().globalData[item] = data
		uni.setStorageSync(item, data)
	}
}

export default gfdGlobalData


export function formatThousandsNew(num, decimal = 2) {
	num = num.toString();
...
export function isBankNo (bankNo = '') {
	var reg = /^[0-9]*$/;
	if (reg.test(bankNo)) {
		return true;
	} else {
