...
const apiUtils = {
	...
	postAjax: function(apiUrl, datas, headers = {
		...
		let requestData = {
			header: {
				"productId": "ZYD001",
				// "iCIFID": uni.getStorageSync("iCIFID"),
				"iCIFID": getApp().globalData.iCIFID,
				"accessChannelNo": "00"
			},
			body: datas
		}
		...
		//将数据进行AES加密
		// var requestDatas = Encrypt(JSON.stringify(requestData), uni.getStorageSync('aesKey'))
		var requestDatas = Encrypt(JSON.stringify(requestData), getApp().globalData.aesKey)
		//将AES加密key进行RSA加密
		// var encodeKeys = rsaUtil.Encrypt(uni.getStorageSync('aesKey'), rsaPublicKey)
		var encodeKeys = rsaUtil.Encrypt(getApp().globalData.aesKey, rsaPublicKey)
		...
		return new Promise((resolve, reject) => {
			request(apiUrl.url, data, "POST", headers).then(res => {
				...
				if (result.body.errorCode == 0 || result.body.errorCode == '000000' || result
...
export function postAjax(apiUrl, datas, headers = {
	...
	return new Promise((resolve, reject) => {
		request(apiUrl.url, data, "POST", headers).then(res => {
			...
			if (result.body.errorCode == 0 || result.body.errorCode == '000000' || result.body
				.errorCode == 'S000A000') {
				console.log('iCIFID: ', result.header.iCIFID)
				getApp().globalData.iCIFID = result.header.iCIFID
				// uni.setStorageSync("iCIFID", result.header.iCIFID)
